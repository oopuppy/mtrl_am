...
def _upload_wandb_videos_from_dir(video_dir: str, uploaded: set, wandb_key: str = "train/video") -> None:
    """video_dir에 있는 mp4/gif 중 아직 안 올린 파일만 W&B에 업로드."""
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # ============================================================
    # 🔹 학습 중간에 비디오를 W&B에 올리기 위한 watcher 스레드
    #    - runner.learn()는 그대로 max_iterations까지 한 번에 돈다.
    #    - 그 동안 background thread가 video_dir을 주기적으로 스캔해서 새 비디오를 wandb.log.
    # ============================================================
    stop_event = None
    watcher_thread = None
    ...
    # run training (원래 구조 그대로 유지)
    try:
        runner.learn(num_learning_iterations=agent_cfg.max_iterations, init_at_random_ep_len=True)
    finally:
        # watcher 중지 + 마지막으로 한 번 더 스캔
