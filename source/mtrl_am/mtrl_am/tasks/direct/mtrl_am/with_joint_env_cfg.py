...
class MtrlAmEnvCfg(DirectRLEnvCfg):
    ...
    randomize_goal_roll_pitch: bool = False
    ...
    # -----------------
    dpos_scale: float = 0.001          # action[0:3] -> delta desired EE position (world)
    dq1_scale: float = 0.0001          # action[3]   -> delta desired Joint_1 angle
    dq2_scale: float = 0.0001          # action[4]   -> delta desired Joint_2 angle
    dyaw_scale: float = 0.01           # action[5]   -> delta desired base yaw

    # (호환용 alias: 기존 코드/실험이 droll/dpitch를 참조할 수 있어서 남겨둠)
    droll_scale: float = dq1_scale
    dpitch_scale: float = dq2_scale

    # -----------------
    # action->desired increment smoothing (LPF time constants)
    #  - 0.0이면 필터 OFF (그대로)
    # -----------------
    cmd_dpos_tau: float = 0.08   # [s]
    cmd_drpy_tau: float = 0.08   # [s]  # 여기서는 [dq1, dq2, dyaw]에 적용
